...
from src.discovery import (
    ...
    baseline_raw_residual_probe,
    baseline_logprob_entropy,
)
...
for layer in config["sae"]["layers"]:
    ...
    print(
        f"CV accuracy: {probe_results['cv_accuracy']:.3f} ± {probe_results['cv_std']:.3f}"
    )
    ...
    consensus_2of3 = (
        (mad_set & probe_set) | (mad_set & freq_set) | (probe_set & freq_set)
    )
    consensus_3of3 = mad_set & probe_set & freq_set

    print(f"Consensus")
    print(
        f"Top-{top_k} overlap (2-of-3 methods): {len(consensus_2of3)} features: {sorted(consensus_2of3)}"
    )
    print(
        f"Top-{top_k} overlap (3-of-3 methods): {len(consensus_3of3)} features: {sorted(consensus_3of3)}"
    )
...
print(
    f"CV accuracy: {logprob_result['cv_accuracy']:.3f} ± {logprob_result['cv_std']:.3f}"
)
...
print(
    f"  Logprob entropy:                   {baseline_results['logprob_entropy']['cv_accuracy']:>8.3f}"
)
...
interpretations_df.to_csv(
    out_dir / "candidate_features_interpretations.csv", index=False
)
print(
    f"Candidate features CSV saved to '{out_dir / 'candidate_features_interpretations.csv'}'"
)
print(
    f"{len(neuronpedia_rows)} consensus features across {len(config['sae']['layers'])} layers"
)
